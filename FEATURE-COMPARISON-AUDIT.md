...
| 随机地图(rmgen) | 图元 100%,地图忠实度 100% | 84/84 全部逐字移植;rmgen2 库与 environment.js 已全量,图专属 rmbiome 已接;新增棘轮测试防退化 |
...
| **地图生成忠实度** | ✅ 84/84 | 全部逐字移植:rmgen2 系 14 张 + 早前 10 张 + 标志性地形与剩余图 60 张(PortF–PortS 批次)。每张图都有自己的 `Generate`/`GenerateRmgen2`,棘轮测试防退化 |
...
| ~~P1~~ ✅ | rmgen 全部 84 张地图的地形结构忠实移植 | 已完成 |
