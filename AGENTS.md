...
### Upstream junctions (OPTIONAL since 2026-09-08 — game runtime no longer needs them)

The C++ engine trees (`binaries/`, `build/`, `libraries/`, `source/`) are **removed from this repo** and can be provided locally as directory junctions pointing at `<0ad upstream>`.

**Game runtime is junction-free**: `RuntimePaths.FindBinariesRoot` probes the in-repo staged root `godot/export/data` (generated by `godot/tools/stage_release_data.sh`) BEFORE the junction. On a fresh clone, run the staging script once and the game runs with no links at all:

```bash
sh godot/tools/stage_release_data.sh /path/to/0ad/binaries/data   # 一次,产出 godot/export/data(1.4G,gitignored)
```

The junctions are still used by: **`dotnet test`** (28 test files walk up to `binaries/data/mods/public` for templates/maps fixtures), **asset pipeline conversions** (`run_full_pipeline.sh` reads upstream art), and **browsing C++/JS reference code**. If you need those, create the links (one-time). Upstream path resolution order on both scripts: explicit argument > `ZEROAD_UPSTREAM` env var > platform default/probe (`C:\SourceCode\0ad` on Windows; `~/SourceCode/gitea/0ad` etc. on macOS/Linux):
...
The junctions/symlinks are gitignored — they will never be tracked. If `<0ad upstream>` moves, just re-run the script (and/or re-run the staging script).
