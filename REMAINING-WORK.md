...
- **GUI**:~~间谍请求(需逐对 LOS 共享基建)~~✅(内核:VisionSharing 组件+SetExtraSeers 全掩码差分+v22 序列化;命令 SpyRequest=33;GUI 面板按钮+spy-failed toast;8 内核测试);~~mod.io minisigs Ed25519 验签~~✅(本波收尾:纯托管 Ed25519+BLAKE2b-512,按原版 ParseSignature/VerifyDownloadedFile 语义——列表期 keynum 匹配+全局签名验签、失败标 invalid,下载后 BLAKE2b-512(文件) 验文件签名、失败与 md5 失配同路径;公钥取 default.cfg [modio] public_key,MinisignVerifier.SelfTest 内置 RFC 8032/7693+原版 test_ModIo.h 向量自检);~~campaigns 末关 endgame 页/useGameSetup 分支~~✅(本波收尾:胜负均走 endgame 流程——胜 markLevelComplete、胜负均收地图脚本自定义结算数据入 run.data;ICampaignGameEndData 钩子=原版 Trigger.prototype.OnCampaignGameEnd;离开回跳战役菜单=原版 nextPage,getMenuPath)。
