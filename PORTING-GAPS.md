...
| M5 GUI | ★★★★★ | 页面基本齐(prelobby/campaigns/viewer/credits/mod 对话框/autostart 全落);GuiInterface 桥热路径收编;小地图面板三钮补齐(空闲工人计数+循环/外交色切换 Alt+V/Flare 信号弹 K+点击+MP 广播互盟过滤);剩引擎级 mod 挂载、阵型行/生产条等低频段 |
...
| M9 rmgen | ★★★★★ | 84/84 生成器全量逐字移植(上游 92 顶层 .js − 8 个 *_triggers 伴随 = 84;注册表键零差异,2026-09-08 复核);库核心+rmgen2+environment+rmbiome 齐;8 张触发器图全落;AllMapsGenerationTests 全绿。注:规划附录 A.4 的"173"为 2026-07 旧盘点口径(含库文件),不反映真值 |
